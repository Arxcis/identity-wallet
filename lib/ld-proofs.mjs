...
function createVerifyHash(options, canonicalizedDocument) {
     ...
     */
    // 3.2) If the proof value parameter, such as jws, exists in options, remove the entry.
    ...
    // 3.3) If created does not exist in options, add an entry with a value that is an [ISO8601] combined date and time string containing the current date and time accurate to at least one second, in Universal Time Code format. For example: 2017-11-13T20:21:34Z.
    if (!"created" in prunedOptions) {
        prunedOptions.created = (new Date()).toISOString()
    }

    // 3.4.1) Creating a canonicalized options document by canonicalizing options
